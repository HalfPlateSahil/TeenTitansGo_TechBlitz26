...
from .lib.pymediasoup.sctp_parameters import SctpStreamParameters
...
from .lib.pymediasoup.data_consumer import DataConsumer
...
# Data Producer
MAX_PAYLOAD_SIZE = 15 * 1024
MAX_BUFFER_SIZE = 65535
RELIABLE_DATA_PRODUCER_LABEL = "reliable_data_producer"
UNRELIABLE_DATA_PRODUCER_LABEL = "unreliable_data_producer"
...
class RoomClient(AsyncIOEventEmitter):
    def __init__(self, **kwargs: Union[RoomClientConfig, Any]):
        ...
        self._closed = False
        self._joined = False
        self._reliable_data_producer = None
        self._unreliable_data_producer = None
        self.__data_consumers: dict[str, DataConsumer] = {}
        self._data_producer_initialized = False
    ...
    async def async_join(self):
        ...
        try:
            if not self._joined:
                ...
                websocket_url = (
                    f"{signaling_url}/?roomId={self.__room_id}&peerId={self.__peer_id}&secret={self.__secret}&mode={self.__mode}&peerType={self.__peer_type}"
                ...
                if self.__produce:
                    ...
                    await self.enable_data_producer()
            ...
            if message.get("method") == "newDataConsumer":
                if self.__consume:
                    await self.handleDataConsume(
                        ...
                        appData=message["data"].get("appData"),
                        peerId=message["data"].get("peerId"),
                    )
                    await self.__accept(message=message)
                else:
    ...
    async def __handle_notifications(self, message: Dict[str, Any]):
        try:
            ...
            if message["method"] == "entryResponded":
                self.peer_id = message["data"].get("peerId") or message["data"].get(
                    "id"
                )
                self.session_id = message["data"]["sessionId"]
                switching_room_id = message["data"].get("switchingRoomId")
                if switching_room_id:
                    if switching_room_id == self.__room_id:
                        logger.info("Same Meeting Id")
                        return
                    else:
                        self.soft_close()
                        await self.switching_room(switching_room_id, mode=self.__mode)
                        return
            ...
            # agent
            if message["method"] == "agentStateChanged":
                logger.debug(f"signal :: agent state changed {message['data']}")
                self.emit(Events.AGENT_STATE_CHANGED, message["data"])

            if message["method"] == "agentTranscript":
                logger.debug(f"signal :: agent transcript {message['data']}")
                self.emit(Events.AGENT_TRANSCRIPT, message["data"])

            if message["method"] == "agentMetrics":
                logger.debug(f"signal :: agent metrics {message['data']}")
                self.emit(Events.AGENT_METRICS, message["data"])
    ...
    async def handle_send_transport(self):
        ...
        @self.__send_transport.on("producedata")
        async def on_producedata(
            ...
            try:
                ...
                logger.error(f"RTC :: error while producedata() :: {e}")
                self.__emit_error("Error while Producing Data")
    ...
    async def handle_join(self, switching_room: bool = False):
        try:
            # Join room
            reqId = self.__generate_random_number()
            req = {
    ...
    async def handleConsume(
        ...
        try:
            ...
            if consumer.track.kind == "audio":
                consumer.track.kind = (
                    "shareAudio"
                    if "share" in consumer.appData and consumer.appData["share"] == True
                    else consumer.track.kind
                )
            else:
                consumer.track.kind = (
                    "share"
            ...
            logger.error(f"RTC :: error while handleConsume() :: {e}")
            self.__emit_error("Error while Handling Consumer")
    ...
    def switch_to(
        ...
        logger.debug(f"RTC :: switching to meeting {meeting_id}")
        self.__tasks[f"{TASK_ANY}-switch-to"] = asyncio.ensure_future(
            self.async_switch_to(meeting_id, local_participant_id)
        )

    async def async_switch_to(
        self,
        meeting_id: str,
        local_participant_id: str,
    ):
        try:
            ...
            await self.__send_request(req)
            logger.debug(f"RTC :: switch to meeting request sent {time.time()}")
            await self.__wait_for(self.__answers[reqId], timeout=15)
        except Exception as e:
            logger.error(f"RTC :: error while switch to meeting request :: {e}")
            self.__emit_error("Error while Switch to Meeting")

    def soft_close(self):
        logger.debug("In soft_close() Start")
        self.__consumers.clear()
        self.emit(Events.SWITCH_ROOM, {"action": "remove_current_meeting_participants"})

    async def switching_room(self, switching_room_id: str, mode: str):
        logger.debug(f"Switching room to {switching_room_id}")
        self.__room_id = switching_room_id
        try:
            joined_data = await self.handle_join(switching_room=True)

            logger.debug("Meeting joined (switched)")
            self.emit(
                Events.MEETING_JOINED,
                {
                    "participants": joined_data["data"]["peers"],
    ...
    async def handleDataConsume(
        ...
        try:
            if self.__recv_transport is None:
                await self.handle_recv_transport()
            logger.debug(f"creating data consumer for participant :: {peerId}")

            data_consumer: DataConsumer = await self.__recv_transport.consumeData(
                id=id,
            ...
            @data_consumer.on("message")
            def on_message(message):
                logger.debug(
                    f'DataConsumer "message" event [streamId:{data_consumer.sctpStreamParameters.streamId}]'
                )
                ...
                if data_consumer.label == RELIABLE_DATA_PRODUCER_LABEL:
                    self.emit(
                        Events.CHAT_MESSAGE,
                        {
                            "payload": payload,
                ...
                elif data_consumer.label == UNRELIABLE_DATA_PRODUCER_LABEL:
                    self.emit(
                        Events.CHAT_MESSAGE,
                        {
                            "payload": payload,
            ...
            @data_consumer.on("transportClose")
            def on_data_consumer_transport_closed():
                logger.debug(f"data consumer transport closed {data_consumer.id}")

            @data_consumer.observer.on("close")
            def on_data_consumer_closed():
                logger.debug(f"data consumer closed {data_consumer.id}")

            logger.debug(f"created data consumer for participant :: {peerId}")

            self.__data_consumers[data_consumer.id] = data_consumer

            return data_consumer

        except Exception as e:
            logger.error(f"RTC :: error while Creating DataConsumer :: {e}")
            self.__emit_error("Error while DataConsume")
    ...
    async def enable_data_producer(self):
        logger.debug("enable_data_producer()")

        try:
            self._reliable_data_producer = await self.__send_transport.produceData(
                ordered=True,
                maxRetransmits=5,
                label=RELIABLE_DATA_PRODUCER_LABEL,
                appData={"info": "my-data-DataProducer"},
            )

            self._reliable_data_producer.bufferedAmountLowThreshold = MAX_BUFFER_SIZE

            self._unreliable_data_producer = await self.__send_transport.produceData(
                ordered=False,
                label=UNRELIABLE_DATA_PRODUCER_LABEL,
                appData={"info": "my-data-DataProducer"},
            )
            self._unreliable_data_producer.bufferedAmountLowThreshold = MAX_BUFFER_SIZE

            logger.info(
                f"_reliable_data_producer={self._reliable_data_producer}, "
                f"_unreliable_data_producer={self._unreliable_data_producer}"
            )

            self._data_producer_initialized = True

            @self._reliable_data_producer.on("transportclose")
            def on_reliable_transport_close():
                self._reliable_data_producer = None
                self._data_producer_initialized = False

            @self._reliable_data_producer.on("open")
            def on_reliable_open():
                logger.debug('chat DataProducer "open" event')

            @self._reliable_data_producer.on("close")
            def on_reliable_close():
                logger.error('chat DataProducer "close" event')
                self._reliable_data_producer = None
                self._data_producer_initialized = False

            @self._reliable_data_producer.on("error")
            def on_reliable_error(error):
                logger.error(f'chat DataProducer "error" event: {error}')

            @self._unreliable_data_producer.on("transportclose")
            def on_unreliable_transport_close():
                self._unreliable_data_producer = None
                self._data_producer_initialized = False

            @self._unreliable_data_producer.on("open")
            def on_unreliable_open():
                logger.debug('unreliable DataProducer "open" event')

            @self._unreliable_data_producer.on("close")
            def on_unreliable_close():
                logger.error('unreliable DataProducer "close" event')
                self._unreliable_data_producer = None
                self._data_producer_initialized = False

            @self._unreliable_data_producer.on("error")
            def on_unreliable_error(error):
                logger.error(f'unreliable DataProducer "error" event: {error}')

        except Exception as error:
            logger.error(f"enable_data_producer() | failed: {error}")
            raise

    async def send(self, payload, opts=None):
        """
        Send data through reliable or unreliable DataProducer.
        """

        opts = opts or {"reliability": ReliabilityModes.RELIABLE.value}
        logger.debug(f"Sending payload: {payload} with opts: {opts}")
        try:

            if not isinstance(payload, (bytes, str)):
                raise TypeError(
                    "Invalid payload type current type is: "
                    + type(payload).__name__
                    + " only string and bytes are allowed"
                )
            ...
            if size > MAX_PAYLOAD_SIZE:
                raise ValueError(
                    f"Payload too large: {size} bytes. Max allowed is {MAX_PAYLOAD_SIZE} bytes"
                )

            if (
                not self.__send_transport
                or self.__send_transport.connectionState != "connected"
            ):
                raise ConnectionError("Cannot send message, Network is not available.")

            if not self._data_producer_initialized:
                logger.debug("Waiting for data producer initialization")

                if not self._data_producer_initialized and self.__use_datachannel:
                    await self.enable_data_producer()

            if opts["reliability"] == ReliabilityModes.RELIABLE.value:
                producer = self._reliable_data_producer
            else:
                producer = self._unreliable_data_producer

            if producer is None:
                raise ValueError("Data producer not initialized.")

            if producer.bufferedAmount > producer.bufferedAmountLowThreshold:
                logger.warning("Buffer full, waiting to resume")
                ...
                def resume_handler():
                    try:
                        ...
                        producer.remove_listener("bufferedamountlow", resume_handler)

                producer.on("bufferedamountlow", resume_handler)
                return await fut
            ...
            logger.error(f"DataProducer.send() failed: {e}")
            raise
    ...
    # Agent methods
    def send_agent_state_changed(self, state):
        self.__tasks[f"{TASK_ANY}-agent-state-changed"] = asyncio.ensure_future(
            self.async_send_agent_state_changed(state)
        )

    async def async_send_agent_state_changed(self, state):
        try:
            reqId = self.__generate_random_number()
            req = {
                "request": True,
                "id": reqId,
                "method": "agentStateChanged",
                "data": {"state": state},
            }
            await self.__send_request(req)
            await self.__wait_for(self.__answers[reqId], timeout=15)
        except Exception as e:
            logger.error(f"Signal :: Error while sending agent state changed :: {e}")
            self.__emit_error("Not Able to Send Agent State Changed")

    def send_agent_transcript(self, data):
        self.__tasks[f"{TASK_ANY}-agent-transcript"] = asyncio.ensure_future(
            self.async_send_agent_transcript(data)
        )

    async def async_send_agent_transcript(self, data):
        try:
            reqId = self.__generate_random_number()
            req = {
                "request": True,
            ...
            logger.error(f"Signal :: Error while sending agent transcript :: {e}")
            self.__emit_error("Not Able to Send Agent Transcript")

    def send_agent_metrics(self, data):
        self.__tasks[f"{TASK_ANY}-agent-metrics"] = asyncio.ensure_future(
            self.async_send_agent_metrics(data)
        )

    async def async_send_agent_metrics(self, data):
        try:
            reqId = self.__generate_random_number()
            req = {
                "request": True,
            ...
            logger.error(f"Signal :: Error while sending agent metrics :: {e}")
            self.__emit_error("Not Able to Send Agent Metrics")
